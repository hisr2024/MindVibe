/**
 * Karma Marg (Karma Journey) Service - Frontend service for the Six Enemies (Shadripu) Journey System.
 *
 * Provides a clean API for interacting with the journey engine backend.
 * All types match backend response models in backend/routes/karma_marg.py
 */
...
const KARMA_MARG_ENDPOINT = '/api/karma-marg';
...
async function apiRequest<T>(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new KarmaMargError(message, code, response.status);
      }

      return data as T;
    } catch (error) {
      if (error instanceof KarmaMargError) {
        throw error;
      }
  ...
  throw new KarmaMargError('Request failed after retries', 'MAX_RETRIES', 503);
}
...
export async function listTemplates(params: ListTemplatesParams = {}): Promise<TemplateListResponse> {
  ...
  const endpoint = queryString
    ? `${KARMA_MARG_ENDPOINT}/templates?${queryString}`
    : `${KARMA_MARG_ENDPOINT}/templates`;
 ...
 * GET /api/karma-marg/templates/{template_id}
 */
export async function getTemplate(templateId: string): Promise<JourneyTemplate> {
  return apiRequest<JourneyTemplate>('GET', `${KARMA_MARG_ENDPOINT}/templates/${templateId}`);
}
...
export async function listJourneys(params: ListJourneysParams = {}): Promise<JourneyListResponse> {
  ...
  const queryString = searchParams.toString();
  const endpoint = queryString
    ? `${KARMA_MARG_ENDPOINT}/journeys?${queryString}`
    : `${KARMA_MARG_ENDPOINT}/journeys`;
 ...
 * GET /api/karma-marg/journeys/{journey_id}
 */
export async function getJourney(journeyId: string): Promise<JourneyResponse> {
  return apiRequest<JourneyResponse>('GET', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}`);
}

/**
 * Start a new journey from a template.
 * POST /api/karma-marg/journeys
 *
 ...
 * POST /api/karma-marg/journeys/{journey_id}/pause
 */
export async function pauseJourney(journeyId: string): Promise<JourneyResponse> {
  return apiRequest<JourneyResponse>('POST', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}/pause`);
}

/**
 * Resume a paused journey.
 * POST /api/karma-marg/journeys/{journey_id}/resume
 */
export async function resumeJourney(journeyId: string): Promise<JourneyResponse> {
  return apiRequest<JourneyResponse>('POST', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}/resume`);
}

/**
 * Abandon a journey.
 * DELETE /api/karma-marg/journeys/{journey_id}
 */
export async function abandonJourney(journeyId: string): Promise<JourneyResponse> {
  return apiRequest<JourneyResponse>('DELETE', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}`);
}
 ...
 * GET /api/karma-marg/journeys/{journey_id}/steps/current
 */
export async function getCurrentStep(journeyId: string): Promise<StepResponse | null> {
  return apiRequest<StepResponse | null>('GET', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}/steps/current`);
}

/**
 * Get a specific step by day index.
 * GET /api/karma-marg/journeys/{journey_id}/steps/{day_index}
 */
export async function getStep(journeyId: string, dayIndex: number): Promise<StepResponse> {
  return apiRequest<StepResponse>('GET', `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}/steps/${dayIndex}`);
}

/**
 * Complete a step.
 * POST /api/karma-marg/journeys/{journey_id}/steps/{day_index}/complete
 */
export async function completeStep(
  ...
  return apiRequest<CompletionResponse>(
    'POST',
    `${KARMA_MARG_ENDPOINT}/journeys/${journeyId}/steps/${dayIndex}/complete`,
    request
  );
}
 ...
 * GET /api/karma-marg/dashboard
 */
export async function getDashboard(): Promise<DashboardResponse> {
  return apiRequest<DashboardResponse>('GET', `${KARMA_MARG_ENDPOINT}/dashboard`);
}
 ...
 * POST /api/karma-marg/fix-stuck-journeys
 */
export async function fixStuckJourneys(): Promise<FixStuckJourneysResponse> {
  return apiRequest<FixStuckJourneysResponse>('POST', `${KARMA_MARG_ENDPOINT}/fix-stuck-journeys`);
}
 ...
 * GET /api/karma-marg/enemies
 */
export async function listEnemies(): Promise<EnemyInfoResponse[]> {
  return apiRequest<EnemyInfoResponse[]>('GET', `${KARMA_MARG_ENDPOINT}/enemies`);
}

/**
 * Get progress for a specific enemy.
 * GET /api/karma-marg/enemies/{enemy}
 */
export async function getEnemyProgress(enemy: string): Promise<EnemyProgressResponse | null> {
  return apiRequest<EnemyProgressResponse | null>('GET', `${KARMA_MARG_ENDPOINT}/enemies/${enemy}`);
}

/**
 * Get radar chart data for all enemies.
 * GET /api/karma-marg/enemies/{enemy}/radar
 ...
 * GET /api/karma-marg/examples/{enemy}
 */
export async function getExamples(enemy: string, limit: number = 4): Promise<ExampleListResponse> {
  return apiRequest<ExampleListResponse>('GET', `${KARMA_MARG_ENDPOINT}/examples/${enemy}?limit=${limit}`);
}

/**
 * Get a random example for an enemy.
 * GET /api/karma-marg/examples/{enemy}/random
 */
export async function getRandomExample(enemy: string): Promise<ExampleResponse | null> {
  return apiRequest<ExampleResponse | null>('GET', `${KARMA_MARG_ENDPOINT}/examples/${enemy}/random`);
}
...
export default karmaMargService;
