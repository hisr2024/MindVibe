...
| Audio — Kiaan Vibe Player meditation tracks (all 23) | **Founder's original work.** Procedurally generated in-browser at playback time by `lib/kiaan-vibe/audio-synth.ts` using the Web Audio API (Om fundamental + tanpura drone, mantra-chant sacred drone, 7-chakra Solfeggio cycle, binaural beats focus/sleep, ambient pads, nature soundscapes — rain/ocean/forest/thunder/stream, zen bowl, samurai, japanese garden). The track catalog in `lib/kiaan-vibe/meditation-library.ts` references `synth://<preset>` URLs — **no recorded audio files** ship for the Player. | Proprietary, all rights reserved | code authored ongoing | founder |
| Audio — Kiaan Vibe Player Gita-verse voice tracks | Track structure + verse-selection + queueing is founder's original work (`lib/kiaan-vibe/gita-voice-tracks.ts`). The voice *rendering* is streamed at request time via the third-party TTS providers per §5; verse text is from the Gita corpus per §1 | Track structure proprietary; voice rendering per upstream TTS terms | runtime | voice-team |
| Audio — `gayatri.mp3` (separate from Vibe Player; used by mantra prompts) | Pixabay — Sounova Music Gayatri Mantra (Pixabay #493174) | Pixabay Content License (CC0-equivalent, commercial use allowed) | 2025-Q4 | voice-team |
| Audio — `om-hanumate.mp3` (separate from Vibe Player; used by mantra prompts) | Pixabay — Kals "Om Hanumate Namaha" (Pixabay #447279) | Pixabay Content License | 2025-Q4 | voice-team |
| Mantras and Vedic chant lyrics (Gayatri Mantra, Om mantras, Hanuman Chalisa-derived phrases, traditional dhyana mantras, etc.) | Traditional sacred texts from antiquity; thousands of years old; **inherently public-domain content**. No author / no living copyright claimant. Modern recordings of these mantras carry recording-specific licenses, but the underlying lyric/melody is not subject to copyright scrutiny. | Public domain (ancient text) | — | — |
