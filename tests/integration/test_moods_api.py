...
class TestMoodsEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_create_mood_success(self, test_client: AsyncClient, test_db: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_create_mood_without_tags_and_note(self, test_client: AsyncClient, test_db: AsyncSession):
        ...
        response = await test_client.post(
            "/moods",
            json={"score": -1},
            headers={"x-auth-uid": "test-mood-user-2"}
        )
    ...
    @pytest.mark.asyncio
    async def test_create_mood_invalid_score(self, test_client: AsyncClient, test_db: AsyncSession):
        ...
        response = await test_client.post(
            "/moods",
            json={"score": 5},
            headers={"x-auth-uid": "test-mood-user-3"}
        )
        
        assert response.status_code == 422  # Validation error
    
    @pytest.mark.asyncio
    async def test_create_mood_creates_user_if_not_exists(self, test_client: AsyncClient):
