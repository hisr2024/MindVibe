...
class TestJournalEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_upload_blob_success(self, test_client: AsyncClient, test_db: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_get_latest_blob_success(self, test_client: AsyncClient, test_db: AsyncSession):
        ...
        assert data["blob_json"] in ['{"encrypted": "first entry"}', '{"encrypted": "second entry"}']
    
    @pytest.mark.asyncio
    async def test_get_latest_blob_empty(self, test_client: AsyncClient, test_db: AsyncSession):
        ...
        response = await test_client.get(
            "/journal/blob/latest",
            headers={"x-auth-uid": "test-journal-user-3"}
        )
    ...
    @pytest.mark.asyncio
    async def test_upload_blob_creates_user_if_not_exists(self, test_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_user_isolation(self, test_client: AsyncClient, test_db: AsyncSession):
