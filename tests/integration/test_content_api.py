...
class TestContentEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_get_content_pack_success(
    ...
    @pytest.mark.asyncio
    async def test_get_content_pack_fallback_to_english(
        self, test_client: AsyncClient, test_db: AsyncSession
    ):
        """Test that non-existent locales fall back to English."""
        # Create only English content pack
        content_pack = ContentPack(
            locale="en", data={"packs": [{"id": 1, "title": "Default Pack"}]}
        )
        test_db.add(content_pack)
        await test_db.commit()
    ...
    @pytest.mark.asyncio
    async def test_get_content_pack_specific_locale(
        self, test_client: AsyncClient, test_db: AsyncSession
    ):
        """Test retrieving locale-specific content."""
        # Create English and Spanish content packs
        en_pack = ContentPack(
            locale="en", data={"packs": [{"id": 1, "title": "English Pack"}]}
        )
        es_pack = ContentPack(
            locale="es", data={"packs": [{"id": 1, "title": "Paquete Español"}]}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_content_pack_structure(
        self, test_client: AsyncClient, test_db: AsyncSession
    ):
