...
def sample_verse():
    ...
    verse.mental_health_applications = {"applications": ["anxiety_management", "stress_reduction"]}
...
class TestChatbotService:
    ...
    async def test_chat_new_session(self, chatbot_service, mock_db, sample_verse):
        ...
        # Mock verse search
        with patch.object(chatbot_service.kb, 'search_relevant_verses', 
                         return_value=[{"verse": sample_verse, "score": 0.9}]):
            with patch.object(chatbot_service, '_generate_chat_response',
    ...
    async def test_template_response_with_verses(self, chatbot_service, sample_verse):
        ...
        assert any(word in response.lower() for word in ["action", "effort", "outcome", "anxiety"])
    ...
    @pytest.mark.asyncio
    async def test_multiple_verses_in_response(self, chatbot_service, mock_db, sample_verse):
...
class TestTemplateResponses:
    ...
    @pytest.mark.parametrize("theme,expected_keywords", [
...
class TestConversationContext:
    ...
    @pytest.mark.asyncio
    async def test_conversation_context_limit(self, chatbot_service, mock_db, sample_verse):
        ...
        with patch.object(chatbot_service.kb, 'search_relevant_verses',
                         return_value=[{"verse": sample_verse, "score": 0.9}]):
            with patch.object(chatbot_service, '_generate_chat_response',
                            return_value="Response") as mock_generate:
                # Send 10 messages
                for i in range(10):
                    await chatbot_service.chat(
                        message=f"Message {i}",
