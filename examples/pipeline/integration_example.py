...
def load_wisdom_verses(filepath: str = None):
    ...
    if filepath is None:
        ...
        filepath = Path(__file__).parent.parent.parent / 'data' / 'wisdom' / 'verses.json'
...
def assess_quality(verses):
    ...
    print(f"   - Verses with principles: {with_principles}/{len(verses)} ({with_principles/len(verses)*100:.1f}%)")
    print(f"   - Verses with keywords: {with_keywords}/{len(verses)} ({with_keywords/len(verses)*100:.1f}%)")
    print(f"   - Verses with suggestions: {with_suggestions}/{len(verses)} ({with_suggestions/len(verses)*100:.1f}%)")
    
    # Find best and worst scores
    if scores:
        best_idx = scores.index(max(scores))
        worst_idx = scores.index(min(scores))
        print(f"   - Best score: {max(scores):.2f} (Verse {verses[best_idx].get('verse_id', 'unknown')})")
        print(f"   - Lowest score: {min(scores):.2f} (Verse {verses[worst_idx].get('verse_id', 'unknown')})")
...
def compare_before_after():
    ...
    verse = {
        ...
        "mental_health_applications": ["anxiety_management", "stress_reduction", "letting_go"]
    }
