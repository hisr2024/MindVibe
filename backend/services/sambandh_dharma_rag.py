...
"""Gita-only retrieval and validation for Sambandh Dharma (Relationship Compass)."""
...
INDEX_PATH = Path("data") / "sambandh_dharma" / "gita_index.sqlite"
EMBEDDING_MODEL = os.getenv("SAMBANDH_DHARMA_EMBEDDING_MODEL", "text-embedding-3-small")
CHAT_MODEL = os.getenv("SAMBANDH_DHARMA_GITA_MODEL", "gpt-4o-mini")
...
class SambandhDharmaIndex:
    ...
    @classmethod
    def load(cls) -> "SambandhDharmaIndex | None":
        if not INDEX_PATH.exists():
            logger.warning("Sambandh Dharma (Relationship Compass) index missing at %s", INDEX_PATH)
            return None

        try:
            conn = sqlite3.connect(INDEX_PATH)
        except sqlite3.Error as exc:
            logger.error("Failed to open Sambandh Dharma (Relationship Compass) index: %s", exc)
            return None
        ...
        logger.info("Loaded Sambandh Dharma (Relationship Compass) index: %d chunks, type=%s", len(chunks), index_type)
        return cls(chunks=chunks, model=model, index_type=index_type)


_cached_index: SambandhDharmaIndex | None = None


def get_index() -> SambandhDharmaIndex | None:
    global _cached_index
    if _cached_index is None:
        _cached_index = SambandhDharmaIndex.load()
...
async def call_ai_provider(messages: list[dict[str, str]]) -> str | None:
    ...
    try:
        ...
        if provider_manager:
            logger.info("Using ProviderManager for Sambandh Dharma (Relationship Compass)")
...
def call_openai_sync(messages: list[dict[str, str]]) -> str | None:
    ...
    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    if not api_key:
        logger.warning("OPENAI_API_KEY not set for Sambandh Dharma (Relationship Compass)")
        return None
    ...
    try:
        ...
        logger.error("Sambandh Dharma (Relationship Compass) OpenAI error: %s", exc)
        return None
