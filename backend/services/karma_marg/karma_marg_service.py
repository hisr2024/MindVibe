"""
Karma Marg (Karma Journey) Service - Multi-Journey Management & Enemy Progress Tracking.
...
Usage:
    from backend.services.karma_marg import KarmaMargService

    service = KarmaMargService(db)
...
class KarmaMargError(Exception):
    """Base exception for Karma Marg (Karma Journey)."""
    pass


class JourneyNotFoundError(KarmaMargError):
    """Journey not found."""
    pass


class TemplateNotFoundError(KarmaMargError):
    """Template not found."""
    pass


class JourneyAlreadyCompletedError(KarmaMargError):
    """Journey is already completed."""
    pass


class StepNotAvailableError(KarmaMargError):
...
class MaxActiveJourneysError(KarmaMargError):
...
class KarmaMargService:
    ...
    async def complete_step(
        ...
        if journey.status != UserJourneyStatus.ACTIVE.value:
            raise KarmaMargError(f"Journey is {journey.status}, cannot complete step")
...
class MultiJourneyManager:
    ...
    def __init__(self, db: AsyncSession):
        self.db = db
        self.engine = KarmaMargService(db)
