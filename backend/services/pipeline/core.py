...
class ContextTransformationPipeline:
    ...
    def create_full_pipeline(cls, strict: bool = False) -> "ContextTransformationPipeline":
        ...
        """
        return cls(enable_sanitization=True, enable_validation=True, enable_enrichment=True, strict_mode=strict)
    ...
    def create_minimal_pipeline(cls) -> "ContextTransformationPipeline":
        ...
        Returns:
            ContextTransformationPipeline instance
        """
        return cls(enable_sanitization=False, enable_validation=True, enable_enrichment=False)
        ...
        try:
            ...
            if self.enable_enrichment and not validation_failed:
                try:
                    ...
                    # Add metadata score
                    result["metadata_score"] = MetadataEnricher.calculate_metadata_score(result)
    ...
    def transform_with_custom_stages(self, verse: dict) -> dict:
        ...
        for stage_name, stage_func in self._custom_stages.items():
            try:
                ...
                if self.strict_mode:
                    raise PipelineError(f"Custom stage '{stage_name}' failed: {e}") from e
