...
# =============================================================================
# ACCELERATED COMPLETION - Tiered response with caching + compressed prompts
# =============================================================================

async def create_accelerated_completion(
    user_message: str,
    mood: str = "neutral",
    topic: str = "general",
    tool: str = "general",
    ...
    """Create response using the 3-tier acceleration strategy.

    Tier 1: Pre-computed wisdom bank (<50ms, 0 API cost)
    Tier 2: Semantic cache (<100ms, 0 API cost)
    Tier 3: Compressed API call (500-800ms, 40-60% fewer tokens)

    All tiers maintain 100% Bhagavad Gita compliance.

    Args:
        user_message: User's raw input
        mood: Detected mood (anxious, angry, sad, confused, overwhelmed, etc.)
        topic: Detected topic (work, relationship, loss, academic, etc.)
        tool: KIAAN tool context (viyoga, relationship_compass, general, etc.)
        conversation_history: Prior messages for context
        model: OpenAI model (default: gpt-4o-mini)
        temperature: Sampling temperature
        max_tokens: Maximum completion tokens
        force_api: Skip cache, force API call

    Returns:
        Dict with keys: content, verses, source_tier, latency_ms, tokens_saved,
        cost_saved_usd, gita_grounded, metrics
    """
    ...
    # ─── Try Tier 1 (pre-computed) and Tier 2 (semantic cache) ───
    accelerated = accelerator.get_accelerated_response(
    ...
    if accelerated:
        logger.info(
            f"Accelerated response from {accelerated.source_tier}: "
            f"{accelerated.latency_ms:.1f}ms, saved {accelerated.tokens_saved} tokens"
        )
        return {
            "content": accelerated.content,
            "verses": accelerated.verses,
            "source_tier": accelerated.source_tier,
            "latency_ms": accelerated.latency_ms,
            "tokens_saved": accelerated.tokens_saved,
            "cost_saved_usd": accelerated.cost_saved_usd,
            "gita_grounded": accelerated.gita_grounded,
            "cached": True,
            "metrics": accelerator.get_metrics(),
        }

    # ─── Tier 3: Compressed API call ────────────────────────
    optimized_request = accelerator.build_optimized_api_request(
        user_message=user_message,
        mood=mood,
        topic=topic,
        tool=tool,
    ...
    try:
        ...
        # Cache this response for future semantic matches
        accelerator.cache_api_response(
            cache_key=optimized_request["cache_key"],
            content=content,
            verses=optimized_request["verses"],
        )

        # Calculate token savings from compression
        standard_tokens = 650  # Average standard prompt tokens
        compressed_tokens = int(optimized_request["tokens_estimate"])
        tokens_saved = max(0, standard_tokens - compressed_tokens)
        cost_saved = (tokens_saved / 1_000_000) * COST_PER_1M_PROMPT_TOKENS

        logger.info(
            f"Compressed API response: {latency_ms:.1f}ms, "
            f"saved ~{tokens_saved} prompt tokens"
        )
        ...
        return {
            ...
            "verses": [
                ...
                for v in optimized_request["verses"]
            ],
            "source_tier": "compressed_api",
        ...
        logger.error(f"Accelerated completion failed: {e}")
        # Fall back to standard completion
        fallback = openai_optimizer.get_fallback_response(tool)
        return {
            "content": fallback,
            "verses": [],
