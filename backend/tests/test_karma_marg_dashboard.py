"""
Tests for KarmaMargService.get_dashboard self-healing behaviour.
...
def _make_service():
    """Build a KarmaMargService with a stub AsyncSession."""
    from backend.services.karma_marg.karma_marg_service import (
        KarmaMargService,
    )

    mock_db = AsyncMock()
    return KarmaMargService(mock_db)
...
async def test_get_dashboard_happy_path(monkeypatch):
    ...
    from backend.services.karma_marg.karma_marg_service import JourneyStats
...
async def test_get_enemy_progress_uses_selectinload(monkeypatch):
    ...
    source = inspect.getsource(KarmaMargService._get_enemy_progress)
...
def test_enemy_progress_response_accepts_active_journey_fields():
    ...
    try:
        from backend.routes.karma_marg import EnemyProgressResponse
    except BaseException as e:  # noqa: BLE001
        pytest.skip(f"routes.karma_marg not importable in this env: {e}")
...
async def test_get_enemy_progress_computes_active_journey_pct():
    ...
    mock_db = AsyncMock()
    service = KarmaMargService(mock_db)
