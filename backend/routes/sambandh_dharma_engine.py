"""Sambandh Dharma Engine - Deep Gita-Grounded Relationship Clarity API.
...
Endpoints:
    POST /api/sambandh-dharma-engine/clarity  - Get relationship clarity
    POST /api/sambandh-dharma-engine/analyze   - Analyze mode and mechanism only
    GET  /api/sambandh-dharma-engine/modes      - List available modes
    GET  /api/sambandh-dharma-engine/health     - Health check
"""
...
from backend.services.sambandh_dharma_engine_prompt import (
...
router = APIRouter(
    prefix="/api/sambandh-dharma-engine",
    tags=["sambandh-dharma-engine"],
)
...
async def engine_health() -> dict[str, Any]:
    """Health check for the Sambandh Dharma Engine with wisdom integration."""
    import os
    ...
    return {
        "status": "ok",
        "service": "sambandh-dharma-engine",
        "version": "2.0",
