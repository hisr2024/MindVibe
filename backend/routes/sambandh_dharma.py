"""Sambandh Dharma (Relationship Compass) - Strict Gita-Grounded Relationship Guidance v3.2.
...
from backend.services.sambandh_dharma_prompt import (
    SAMBANDH_DHARMA_GITA_SYSTEM_PROMPT,
    SAMBANDH_DHARMA_SECULAR_PROMPT,
)
from backend.services.sambandh_dharma_rag import (
...
from backend.services.sambandh_dharma_storage import (
    CompassMessage,
    append_message,
    ensure_session,
    get_recent_messages,
...
from backend.services.gita_wisdom_retrieval import (
    search_gita_verses,
    build_gita_context,
    generate_sambandh_dharma_fallback,
    is_ready as gita_ready,
    get_verses_count,
)
from backend.services.sambandh_dharma_analysis import (
...
from backend.services.sambandh_dharma_synthesizer import get_wisdom_synthesizer
from backend.services.sambandh_dharma_engine import (
    ai_analysis as engine_ai_analysis,
)

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/sambandh-dharma", tags=["sambandh-dharma"])
...
class UnifiedCompassRequest(BaseModel):
    """Request schema for the unified secular Sambandh Dharma (Relationship Compass) with Wisdom Core."""
    model_config = ConfigDict(populate_by_name=True)
    message: str = Field(
...
try:
    ...
    logger.info("✅ Sambandh Dharma (Relationship Compass) v3.0: WellnessModel initialized with Gita-grounding")
except Exception as e:
    logger.warning(f"⚠️ Sambandh Dharma (Relationship Compass): WellnessModel unavailable: {e}")

# Log Gita verses availability
if gita_ready():
    logger.info(f"✅ Sambandh Dharma (Relationship Compass) v3.0: {get_verses_count()} Gita verses available for retrieval")
else:
    logger.warning("⚠️ Sambandh Dharma (Relationship Compass): Gita verses not loaded - using RAG/fallback")
...
async def _fetch_dynamic_wisdom(
    ...
    try:
        ...
        if dynamic_items:
            logger.info(
                f"SambandhDharma: Retrieved {len(dynamic_items)} dynamic wisdom items "
...
async def get_relationship_guidance(
    ...
    if ai_analysis.analysis_depth == "ai_enhanced" and ai_analysis.confidence >= 0.5:
        ...
        logger.info("SambandhDharma: Using AI-enhanced search query")
    else:
    ...
    if gita_verses:
        gita_context, sources = build_gita_context(gita_verses, tool="sambandh_dharma")
        ...
        logger.info(
            f"SambandhDharma: Retrieved {len(gita_verses)} static verses "
        ...
        logger.info("SambandhDharma: Using core relationship Gita wisdom")
    ...
    if not wellness_model:
        logger.warning("SambandhDharma: WellnessModel unavailable, using Gita fallback")
    ...
    try:
        ...
        result = await wellness_model.generate_response(
            tool=WellnessTool.SAMBANDH_DHARMA,
        ...
        logger.info(
            f"✅ SambandhDharma guidance: {analysis_mode.value} mode, "
        ...
        logger.exception(f"Sambandh Dharma (Relationship Compass) error: {e}")
        return _get_gita_grounded_fallback(
            conflict, gita_verses, relationship_type, primary_emotion, analysis_mode
        )
...
async def get_relationship_guidance_gita_only(
    ...
    if ai_analysis.analysis_depth == "ai_enhanced" and ai_analysis.confidence >= 0.5:
        ...
        logger.info("SambandhDharma: Using AI-enhanced query for wisdom retrieval")
    else:
    ...
    system_prompt = SAMBANDH_DHARMA_SECULAR_PROMPT if secular_mode else SAMBANDH_DHARMA_GITA_SYSTEM_PROMPT
    ...
    if not response_text or not is_valid:
        logger.warning("Sambandh Dharma (Relationship Compass) AI failed, using fallback: %s", errors)
...
@router.get("/health")
async def sambandh_dharma_health():
    ...
    return {
        ...
        "ai_analysis": {
            ...
            "model": os.getenv("SAMBANDH_DHARMA_ANALYSIS_MODEL", "gpt-4o-mini"),
            "capabilities": [
                "emotional_analysis",
...
def _get_gita_grounded_fallback(
    ...
    fallback_result = generate_sambandh_dharma_fallback(
        user_input=conflict,
        verses=gita_verses,
        relationship_type=relationship_type.value,
    )
...
async def unified_health() -> dict[str, Any]:
    """Health check for the unified secular Sambandh Dharma (Relationship Compass)."""
    import os
    ...
    return {
        "status": "ok",
        "service": "sambandh-dharma-unified",
        "version": "5.0",
