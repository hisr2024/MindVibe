...
async def get_latest_assessment(
    ...
    verse_details = await _fetch_verse_details(db, assessment.personalized_verses or [])
...
def _determine_focus_areas(
    responses: dict[str, Any], scores: dict[str, Any]
) -> list[str]:
...
async def _get_personalized_verses(
    db: AsyncSession, responses: dict[str, Any], scores: dict[str, Any]
) -> list[dict[str, int]]:
