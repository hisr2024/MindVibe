...
class Settings(BaseSettings):
    ...
    # ----------------------------------------------------------------------
    # Sakha (Journey completion) AI response flag
    # ----------------------------------------------------------------------
    # When False (default), the journey-engine's _build_sakha_response()
    # composes a deterministic templated reply from _ENEMY_SACRED. When
    # True, the response is generated by the shared provider_manager
    # (OpenAI / Sarvam / fallback) and is REQUIRED to ground itself
    # exclusively in the Wisdom Core: the 700+ verse GitaWisdomCore corpus
    # plus ModernExamplesDB. The contract returned to the client is
    # identical in both modes — only the body text changes — so
    # turning this on (or off) never breaks Android / web.
    ENABLE_AI_SAKHA_RESPONSE: bool = parse_bool_strict(
        os.getenv("ENABLE_AI_SAKHA_RESPONSE", "false"), "ENABLE_AI_SAKHA_RESPONSE"
    )
    # Tunables. Defaults are conservative so we never spend more than ~600
    # tokens or 8s on a single completion call, and so the model can't
    # rewrite multi-paragraph essays.
    AI_SAKHA_MAX_TOKENS: int = int(os.getenv("AI_SAKHA_MAX_TOKENS", "320"))
    AI_SAKHA_TEMPERATURE: float = float(os.getenv("AI_SAKHA_TEMPERATURE", "0.4"))
    AI_SAKHA_TIMEOUT_SECS: float = float(os.getenv("AI_SAKHA_TIMEOUT_SECS", "8"))
    AI_SAKHA_MAX_BODY_CHARS: int = int(os.getenv("AI_SAKHA_MAX_BODY_CHARS", "650"))
