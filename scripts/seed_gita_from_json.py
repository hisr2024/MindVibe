    ...
    async with AsyncSessionLocal() as session:
        for verse in verses_data:
            ...
            existing_verse = await session.execute(
                select(GitaVerse).filter_by(chapter=verse["chapter"], verse=verse["verse"])
            )
        ...
        try:
            ...
            print(f"Seeded {seeded_count} verses, skipped {skipped_count}, failed {failed_count}")
        except Exception as e:
...
async def verify_seeding():
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(GitaVerse.chapter, func.count(GitaVerse.id)).group_by(GitaVerse.chapter))
