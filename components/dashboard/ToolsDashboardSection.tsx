...
interface ToolsDashboardSectionProps {
  onKiaanClick?: () => void
}

export function ToolsDashboardSection({ onKiaanClick }: ToolsDashboardSectionProps) {
  return (
          ...
          >
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {category.tools.map(tool => {
                // Special handling for KIAAN - open modal instead of navigating
                if (tool.id === 'kiaan' && onKiaanClick) {
                  return (
                    <button
                      key={tool.id}
                      onClick={onKiaanClick}
                      className="group flex items-center justify-between gap-2 rounded-xl border border-white/5 bg-black/30 px-3 py-2 shadow-[0_10px_30px_rgba(0,0,0,0.25)] transition hover:border-orange-400/40 hover:bg-black/40 focus:outline-none focus:ring-2 focus:ring-orange-500/60 focus:ring-offset-2 focus:ring-offset-[#0b0b0f] text-left w-full"
                    >
                ...
                // Default Link behavior for all other tools
                return (
                  <Link
                    key={tool.id}
                    href={tool.href}
