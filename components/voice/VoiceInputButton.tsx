/**
 * Voice Input Button — Shankha (Sacred Conch) Voice-to-Text
 *
 * A standalone voice-to-text button using the Shankha icon (Vishnu's conch).
 * Built from scratch as a clean, self-contained entity.
 *
 * States:
 *   - Idle: Shankha icon, gold border, ready to record
 *   - Listening: Shankha filled + pulsing red border, recording active
 *   - Denied: Shankha with slash overlay, permission guidance on click
 *
 * Integration: Drop into any input area via onTranscript callback.
 */
...
export function VoiceInputButton({
  ...
  useEffect(() => {
    ...
    navigator.permissions
      ...
      .then((status) => {
        ...
        status.onchange = () => {
          setPermission(status.state as PermissionState)
          if (status.state === 'granted') setShowHint(false)
        }
      })
      .catch(() => {
  ...
  return (
    <div className="relative group">
        ...
        title={
          isDenied
            ...
            : isListening
              ...
              : 'Click to speak — Shankha Voice Input'
        }
      >
        <ShankhaIcon
          size={20}
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 space-y-1.5">
              ...
              <p className="text-[11px] text-[#f5f0e8]/70 leading-relaxed">
                To use Shankha voice input, allow microphone access in your browser
      ...
      {/* ── Hover hint (when permission not yet granted) ── */}
      {!isListening && !error && !isDenied && permission !== 'granted' && (
        <div className="absolute right-0 top-full mt-2 z-20 min-w-[200px] max-w-[280px] rounded-lg bg-slate-900/95 px-3 py-2 text-xs text-[#f5f0e8]/70 shadow-lg border border-[#d4a44c]/30 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
          ...
          permission. Audio is processed locally.
        </div>
      )}
    </div>
  )
