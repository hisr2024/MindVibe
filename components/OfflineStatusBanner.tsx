 ...
 *
 * Performance: useCacheManager (7 IndexedDB queries) is only invoked when
 * the user is actually offline, avoiding 100-300ms of render-blocking I/O
...
/**
 * Inner component that calls useCacheManager — only rendered when offline.
 * This avoids running 7 IndexedDB queries on every page mount because the
 * parent component conditionally renders this only when !isOnline.
 */
...
export function OfflineStatusBanner() {
  ...
  return (
    <AnimatePresence>
      {!isOnline && (
        <OfflineBannerContent queueCount={queueCount} lastSyncTime={lastSyncTime} />
      )}
    </AnimatePresence>
  )
}
