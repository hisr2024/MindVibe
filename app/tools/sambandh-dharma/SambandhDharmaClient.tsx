...
export default function SambandhDharmaClient() {
  const { t, language } = useLanguage()
  const [conflict, setConflict] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [result, setResult] = useLocalState<SambandhDharmaResult | null>('sambandh_dharma', null)
  const [sessionId, setSessionId] = useLocalState<string>('sambandh_dharma_session', '')
  ...
  async function requestCompass() {
    ...
    try {
      ...
      if (!response.ok && !data.response) {
        setError('Sambandh Dharma (Relationship Compass) is gathering wisdom. Please try again shortly.')
        return
      }
      ...
      if (guidance && typeof guidance === 'object') {
        ...
        setError('Sambandh Dharma (Relationship Compass) could not generate a response. Please try again.')
      }
    } catch (err) {
      if (err instanceof DOMException && err.name === 'AbortError') return
      setError('Unable to reach Sambandh Dharma (Relationship Compass). Check your connection and retry.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const leftContent = (
    <>
      ...
      {loading && <WisdomLoadingState tool="sambandh_dharma" secularMode={false} />}
  ...
  const rightContent = (
    <>
      ...
      <SpiritualToolsNav currentTool="sambandh-dharma" />
      <CompanionCTA fromTool="sambandh-dharma" />
    </>
  )
  ...
  return (
    <SacredPageShell
      ...
      modeLabel={`${t('dashboard.mode_label.prefix', 'You are in:')} ${t('dashboard.mode_label.sambandh-dharma', 'Relationship Mode')}`}
      verse={{
        english: '',
