...
import { karmaMargService, KarmaMargError } from '@/services/karmaMargService'
...
export default function JourneyDetailPage() {
  ...
  const loadStep = async (dayIndex: number) => {
    try {
      const step = await karmaMargService.getStep(journeyId, dayIndex)
      setCurrentStep(step)
  ...
  const handleCompleteStep = async () => {
    ...
    try {
      const result = await karmaMargService.completeStep(journeyId, currentStep.day_index, {
      ...
      if (result.journey_complete) {
        ...
        setTimeout(async () => {
          ...
          // Load the next step
          try {
            const nextStep = await karmaMargService.getStep(journeyId, result.next_day!)
