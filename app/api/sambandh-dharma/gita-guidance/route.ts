...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/sambandh-dharma/gita-guidance`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        console.error(`[Sambandh Dharma (Relationship Compass) Gita] Backend returned ${response.status}: ${errorText}`)
      ...
      if (backendError instanceof Error && backendError.name === 'AbortError') {
        console.warn('[Sambandh Dharma (Relationship Compass) Gita] Request timeout')
      } else {
        console.warn('[Sambandh Dharma (Relationship Compass) Gita] Backend connection failed:', backendError)
      }
      return NextResponse.json(FALLBACK_RESPONSE)
    }
  } catch (error) {
    console.error('[Sambandh Dharma (Relationship Compass) Gita] Error:', error)
    return NextResponse.json(FALLBACK_RESPONSE)
  }
}
