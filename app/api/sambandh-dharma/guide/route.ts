/**
 * Sambandh Dharma (Relationship Compass) Guide API Route (Gita-grounded wisdom proxy)
 *
 * Proxies to the backend Sambandh Dharma Engine which integrates
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const engineUrl = `${BACKEND_URL}/api/sambandh-dharma-engine/clarity`
      const legacyUrl = `${BACKEND_URL}/api/sambandh-dharma/gita-guidance`
      ...
      if (!response.ok) {
        ...
        console.error(`[Sambandh Dharma (Relationship Compass) Guide] Backend (${targetUrl}) returned ${response.status}: ${errorText}`)
      ...
      if (backendError instanceof Error && backendError.name === 'AbortError') {
        console.warn('[Sambandh Dharma (Relationship Compass) Guide] Request timeout')
      } else {
        console.warn('[Sambandh Dharma (Relationship Compass) Guide] Backend connection failed:', backendError)
      }
      return NextResponse.json(FALLBACK_RESPONSE)
    }
  } catch (error) {
    console.error('[Sambandh Dharma (Relationship Compass) Guide] Error:', error)
    return NextResponse.json(FALLBACK_RESPONSE)
  }
}
