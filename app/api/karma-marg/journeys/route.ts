/**
 * Karma Marg (Karma Journey) Journeys API Proxy
 *
 * GET  /api/karma-marg/journeys → List user journeys (with fallback)
 * POST /api/karma-marg/journeys → Start a new journey from a template
 *
 * Proxies to the backend karma-marg service. Returns an empty
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetchWithRetry(
      `${BACKEND_URL}/api/karma-marg/journeys${url.search}`,
      {
        method: 'GET',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetchWithRetry(
      `${BACKEND_URL}/api/karma-marg/journeys`,
      {
        method: 'POST',
