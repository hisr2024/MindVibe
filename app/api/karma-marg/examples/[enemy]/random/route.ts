/**
 * Karma Marg (Karma Journey) Random Example by Enemy API Proxy
 *
 * GET /api/karma-marg/examples/[enemy]/random → Get a random example journey for a specific enemy
 */
...
export async function GET(
  ...
  try {
    const response = await fetchWithRetry(
      `${BACKEND_URL}/api/karma-marg/examples/${encodeURIComponent(enemy)}/random`,
      { method: 'GET', headers: proxyHeaders(request, 'GET') },
      { maxRetries: 1, timeoutMs: 15000, label: '[Journey GET /examples/:enemy/random]' }
    )
