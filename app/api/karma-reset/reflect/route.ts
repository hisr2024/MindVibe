/**
 * Karma Reset Reflection API Route
 * Returns adaptive reflection questions based on the user's karma context.
 * Uses AI generation with fallback to static questions per category.
 */

import { NextRequest, NextResponse } from 'next/server'
import { proxyHeaders, BACKEND_URL, forwardCookies } from '@/lib/proxy-utils'

/** Static fallback questions per category when AI is unavailable */
const FALLBACK_QUESTIONS: Record<string, { question: string; subtext: string; options: string[] }[]> = {
  action: [
    {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { context, questionIndex } = body

    if (!context?.category || questionIndex === undefined) {
      return NextResponse.json(
        { detail: 'Missing required fields: context.category, questionIndex' },
        { status: 400 }
      )
    }
    ...
    try {
      const backendResponse = await fetch(`${BACKEND_URL}/api/karma-reset/reflect`, {
        method: 'POST',
        headers: proxyHeaders(request, 'POST'),
        body: JSON.stringify({ context, questionIndex }),
    ...
    const questions = FALLBACK_QUESTIONS[category] || FALLBACK_QUESTIONS.action
    const idx = Math.min(questionIndex, questions.length - 1)

    return NextResponse.json(questions[idx])
  } catch (error) {
    console.error('[Karma Reset Reflect] Error:', error)
    return NextResponse.json(
      { detail: 'Unable to generate reflection question' },
      { status: 500 }
    )
  }
}
